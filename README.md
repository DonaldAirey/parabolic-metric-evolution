...
$$\Delta s^1 = \Delta x_o^1 - \frac{1}{2}\left(\Delta x_o^1 - \Delta x_e^1\right)$$
$$\Delta s^1 = \frac{1}{2}\left(\Delta x_o^1 + \Delta x_e^1\right)$$
<p align="center">
...
<p>
The remaining quadratic form is constrained up to a positive overall factor and a relative scaling between the temporal and spatial functionals. These functionals are identified with the canonical imaginary and real components of the complex manifold, fixing their relative normalization. The bilocal interval is therefore:
</p>
...
<p>
The intrinsic evolution is defined along $\chi$, while observable dynamics are expressed in the projected time coordinate $t$ with $d\chi = i\,dt$. Differentiating the manifold extent twice with respect to $t$ gives:
</p>
$$\frac{d^2 L}{dt^2} = -A$$
<p>
...
<p>
Substituting the temporal and spatial separations into the bilocal interval yields:
</p>
...
<p>
Identifying wavelength with a comoving spatial separation and assuming such separations scale with the manifold extent gives:
</p>
...
$$t_e = \frac{V_0(1+z) - \sqrt{(1+z)\left[(V_0 - A t_o)^2 + V_0^2 z\right]}}{A(1+z)}$$
<p>
...
<p>
The null structure of the bilocal interval determines the extent of causal connectivity on the manifold. Setting the emission event at the origin of the manifold history $t_e = 0$ and the observation event at epoch $t_o = t$, the causal horizon is:
</p>
$$d_{\mathrm{ch}}(t) = t\left(2V_0 - A t\right)$$
<p>
Differentiating gives the expansion rate of the causal boundary:
</p>
$$c_{\mathrm{c}}(t) = \frac{d}{dt},d_{\mathrm{ch}} = 2\left(V_0 - At\right)$$
<p>
This quantity defines the rate at which the accessible causal domain can be extended under the operational clock, and characterizes the growth of the causally orderable region.
</p>
<p>
...
<p>
The quadratic term is governed by the constant value $\ddot{L}(t_0) = -A$. The local reduction therefore contains a homogeneous second-order contribution corresponding to a uniform deceleration of magnitude $A$ inherited from the global manifold evolution, which provides the physical origin of gravity in the local limit.
</p>
<p>
...
<p>
The background term $a_{\mathrm{bg}}$ is uniform and has constant magnitude $A$. In the absence of localized content it defines the manifold evolution, and an inertial frame follows it without inducing concentration or depletion. Localized inertial content resists this acceleration, redistributing the flux into spatial concentrations that define the sourced gravitational component $a_{\mathrm{conc}}(x)$ governing bound dynamics.
</p>
...
<p>
We adopt a minimal closure as the lowest-order local form consistent with locality, linearity in enclosed inertial content, rotational invariance, and additivity. These conditions restrict the sourced field to a divergence relation as the leading-order connection between field structure and inertial content. For a spherically symmetric configuration the flux through a sphere of radius $r$ is:
</p>
$$\Phi(r) = \oint_S \mathbf{a}_{\mathrm{conc}} \cdot d\mathbf{S} = 4\pi r^2 a_r(r)$$
<p>
Imposing flux conservation together with linearity in the enclosed inertial content gives:
</p>
$$\oint_S \mathbf{a}_{\mathrm{conc}} \cdot d\mathbf{S} = -4\pi G M(r)$$
<p>
where $M(r)$ is the enclosed inertial content within radius $r$ and $G$ is the empirical coupling relating inertial content to concentration of the background acceleration. Spherical symmetry then yields:
</p>
$$\mathbf{a}_{\mathrm{conc}}(r) = -\frac{G M(r)}{r^2},\hat{r}$$
<p>
and Gauss's theorem gives the Poisson equation for the sourced weak-field limit:
</p>
$$\nabla \cdot \mathbf{a}{\mathrm{conc}} = -4\pi G \rho{\mathrm{inertial}}$$
<p>
...
<p>
For a body in a circular orbit of radius $r$ with tangential speed $v$, equating the centripetal acceleration with the inward acceleration and solving for the enclosed baryonic mass gives:
</p>
$$M_b(r) = \frac{r\left(v^2 - A r\right)}{G}$$
<h3>Circular-Orbit Solutions</h3>
<p>
The equation above defines the PME fundamental plane, a two-parameter family of circular-orbit solutions relating baryonic mass $M$, orbital radius $r$, and orbital speed $v$. For fixed orbital speed $v$, the mass function reaches a maximum at:
</p>
$$r_{\max} = \frac{v^2}{2A}$$
<p>
...
<p>
The surface $M(v, r)$ therefore defines a fundamental plane of circular-orbit solutions. The ridge $M_{\max}(v)$ gives the upper envelope of baryonic mass for a given orbital speed, and galaxies dominated by circular rotation are expected to saturate this boundary.
</p>
<p align="center">
...
$$A = 3.7 \times 10^{-11}\ \mathrm{m,s^{-2}}$$
<p align="center">
  <img src="figures/btfr-chart.png" alt="Baryonic Tully–Fisher relation" width="700">
</p>
<p align="center">
  <em>
    Log–log plot of baryonic mass $M_b$ versus outer rotation speed $v$ for the SPARC galaxy sample.
    Black circles show the observed galaxies. The solid red line shows the PME circular-orbit envelope
    evaluated at the best-fit acceleration.
  </em>
</p>
<h3>Global Acceleration Budget</h3>
<p>
The local closure fixes all non-homogeneous acceleration flux as sourced by inertial content. Integrating the flux law over the full homogeneous domain of scale $L$ and using the global relation between baryonic mass and the background acceleration scale gives:
</p>
...
<h3>Mach's Bucket</h3>
<p>
The bilocal reduction introduces a homogeneous acceleration scale $A$ fixed by the geometry of the manifold. This acceleration is not sourced by baryonic matter; instead, baryonic mass redistributes it through conservation of acceleration flux. Because the total acceleration is finite, this redistribution cannot be local: any concentration within the system must be balanced globally. Rotational inertia therefore depends on the global distribution of mass through the shared acceleration field, providing a Machian interpretation.
</p>
...
<p>
so that $\Delta t = \Delta x\,/\,(V_0 - At)$. This defines the null boundary of the local geometry and fixes the relation between spatial separation and temporal increment through the scale $(V_0 - At)$.
</p>
<p>
...
<h3>Proper Time</h3>
<p>
Using the local interval, proper time defines the invariant timelike interval along admissible trajectories, $d\tau^2 \equiv -ds^2$:
</p>
...
<p>
A non-zero manifold acceleration defines a global acceleration flux that selects a timelike direction. In the homogeneous local limit, orientation relative to the flux is represented by an angle $\theta$. Resolving the tangent via the local invariant, the orthogonal projections are:
</p>
$$dx = (V_0 - At)\sin\theta,d\lambda, \qquad dt = \cos\theta,d\lambda$$
<p>
The measurable velocity is:
</p>
$$v = \frac{dx}{dt} = (V_0 - At)\tan\theta$$
<p>
The null boundary is reached when $v = V_0 - At$, requiring $\theta = \pi/4$. Observable motion is accordingly restricted to the timelike domain $0 \leq \theta < \pi/4$. Momentum is defined from the observable velocity:
</p>
$$P = m,v$$
<p>
...
<h3>Local Action</h3>
<p>
In the coincidence limit, weighting the invariant interval $ds$ by the tangent momentum scale $P_{\mathrm{geom}} \equiv m(V_0 - At)$ defines the invariant geometric action increment:
</p>
$$dS_{\mathrm{geom}}^2 = P_{\mathrm{geom}}^2,ds^2 = m^2(V_0 - At)^2\left[-(V_0 - At)^2 dt^2 + dx^2\right]$$
<p>
The temporal component defines the geometric energy scale:
</p>
...
$$dS = P,dx - H,dt$$
<p>
which defines the canonical momentum and observable Hamiltonian. The observable action plays the role of the Hamilton–Jacobi principal function, while the invariant geometric action remains well-defined across the full manifold.
</p>
<h3>Phase Evolution</h3>
<p>
At the null boundary, $ds^2 = 0$ and $d\tau \to 0$, marking the limit of observable timelike motion. Beyond this boundary, observable motion ceases, while the invariant geometric structure remains well-defined and the geometric action continues to accumulate along the manifold tangent. The continued evolution of this invariant scalar is represented as phase:
</p>
...
<p>
The luminosity distance follows from the standard relation $D_L(z) = (1+z)D_C(z)$. Substituting the spatial separation yields the analytic luminosity distance:
</p>
$$D_L(z) = -\frac{t_o\left(A t_o - 2V_0\right) z(1+z)}{2+z}$$
<h3>Distance Modulus</h3>
<p>
Type Ia supernova observations are reported in terms of the distance modulus:
</p>
...
$$V_0 = c_o + A t_o$$
<p>
With the manifold acceleration $A$ determined from the baryonic Tully–Fisher analysis, the Cepheid-calibrated Pantheon+ subset is used to determine the observation epoch $t_o$ on the absolute distance scale. The best-fit observation epoch is obtained by minimizing the covariance-weighted chi-square statistic $\chi^2 = r^{\mathrm{T}} C^{-1} r$, where $C$ is the full Pantheon+ STAT+SYS covariance matrix. The resulting parameters are:
</p>
Parameter	Value
$A$	$3.7 \times 10^{-11}\ \mathrm{m,s^{-2}}$
$V_0$	$3.16 \times 10^{8}\ \mathrm{m,s^{-1}}$
$t_o$	$4.51 \times 10^{17}\ \mathrm{s}$
...
<p>
Evaluating at the observation epoch $t = t_o$ using the best-fit manifold parameters gives the present-day expansion rate as a prediction rather than a fitted parameter:
</p>
...
$$\rho_b = 9.49 \times 10^{-28}\ \mathrm{kg,m^{-3}}$$
<p>
...
<p>
The photon density is determined from the measured CMB temperature $T_\gamma = 2.725\ \mathrm{K}$. The mapping between temperature and photon density depends on the local geometric scale $(V_0 - At)$, so the photon density is treated as epoch dependent. For a blackbody radiation field, the photon number density is:
</p>
$$n_\gamma(t) = \frac{2\zeta(3)}{\pi^2}\left(\frac{k_B T_\gamma}{\hbar(V_0 - At)}\right)^3$$
<p>
and the mass-equivalent photon density becomes:
</p>
...
<p>
The Thomson scattering rate, incorporating the local geometric scale, is:
</p>
$$\Gamma_T(t) = n_e(t),\sigma_T,(V_0 - At)$$
<p>
The optical depth and visibility function are:
</p>
$$\tau_T(t) = \int_t^{t_o} \Gamma_T(t'),dt', \qquad g(t) = \Gamma_T(t),e^{-\tau_T(t)}$$
<p>
...
<h3>Sound Horizon</h3>
<p>
Prior to recombination, baryons and photons form a tightly coupled fluid whose acoustic interaction distance defines the sound horizon. The baryon–photon momentum ratio is:
</p>
$$R(t) = \frac{3\rho_b}{4\rho_\gamma(t)},a(t)$$
<p>
giving the sound speed:
</p>
$$c_s(t) = \frac{c_{\mathrm{c}}(t)}{\sqrt{3\left(1 + R(t)\right)}}$$
<p>
The causal expansion rate $c_{\mathrm{c}}(t)$ sets the effective causal scale entering the acoustic horizon integral, so that the sound horizon is:
</p>
...
<p>
The observed acoustic angle is determined by the ratio of the sound horizon at recombination to the bilocally assigned transverse size of the emission–observation pair. Applying the bilocal spatial rule to the transverse configuration defines the effective bilocal transverse circumference:
</p>
$$C_\perp \equiv \frac{1}{2}\left[L(t_o) + L(t_)\right] = \frac{L(t_o)}{2},\frac{2 + z_}{1 + z_*}$$
<p>
...
$$\theta_* = \frac{r_s}{r_\perp} = \frac{8\pi(1 + z_)}{t_o(2V_0 - A t_o)(2 + z_)},r_s$$
<p>
Evaluating with the manifold parameters gives:
</p>
